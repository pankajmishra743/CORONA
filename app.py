...
for i in range(1,10):
    if(str(d[i].iloc[0][1]) == "Andaman and Nicobar Islands"):
        df = d[i].iloc[:-2]
    else:
        df = ''
    if(str(d[i].iloc[0][0]) == "Jan-30"):
        state_df  = d[i].iloc[:-4]
    else:
        state_df = ''
...
def sample_metadata(state):
    ...
    AllIndia_metadata['TOTAL'] = ldf[ldf['STATE_UT']==state]['RECOVERIES'].to_string(index=False).replace("#", "")
